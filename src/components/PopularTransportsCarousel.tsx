...
export default function PopularTransportsCarousel({
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setLoading(true);
        const res = await fetch("/api/transports/popular", { cache: "no-store" });
  ...
  return (
    <section className="px-5 md:container">
      <div className="flex items-center justify-between px-2">
        <h5 className="text-center">Rent Transportation</h5>
        <Link
          href={seeMoreHref}
        ...
        <div className="-mx-2 sm:-mx-3 md:-mx-4">
          <div className="embla overflow-hidden container px-2 sm:px-3 md:px-4" ref={emblaRef}>
            <div className="embla__container flex gap-2 sm:gap-3 md:gap-4">
              {loading && (
                <div className="px-4 py-8 text-sm text-gray-500">Loading vehicles…</div>
              )}
              {!loading && items.length === 0 && (
                <div className="px-4 py-8 text-sm text-gray-500">No transports available.</div>
              )}
                    ...
                    <div className="h-full flex flex-col gap-1 rounded-md border-2 border-gray-200 relative bg-white">
                      ...
                      <div className="flex flex-1 flex-col px-3 pt-2 pb-4">
                        ...
                        <div className="mt-1 flex items-center gap-1">
                          <p>
                            <span className="text-[6px]">AED</span> {fmtMoney(v.ratePerDay)}/day
                          </p>
                          <span>|</span>
                          <p>
                            <span className="text-[6px]">AED</span> {fmtMoney(v.ratePerHour)}/hr
                          </p>
                        </div>
                        ...
                        <div className="flex justify-between items-center">
                          ...
                          {!!v.passengers && (
                            <p className="mt-1 text-md text-black/60">seats: {v.passengers}</p>
                          )}
                        </div>
